...
from .chunk import approx_token_count
...
def guess_lang_from_path(path: str | Path) -> Tuple[str, str]:
    ...
    if ext in CODE_EXTS:
        ...
        kind = "doc"
    lang = EXT_LANG.get(ext, (ext[1:] if ext.startswith(".") and len(ext) > 1 else "text"))
    return kind, lang
...
def build_record(
    *,
    ...
    kind, lang_hint = guess_lang_from_path(rp)
    if not lang:
    ...
    tokens = approx_token_count(text, "code" if kind == "code" else "doc")
