...
def target_band(lang: str) -> tuple[int,int]:
    ...
    if lang in CODE_LANGS: return (2000, 4000)
    if lang in LOG_LIKE:   return (1000, 2000)
    if lang in TEXTY:      return (1500, 2000)
...
class JSONLQualityScorer:
    ...
    def score_record(self, rec: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sh = simhash64(text)
        near_dup = any(hamming(sh, h) < self.sim_thresh for h in self.sim_seen)
        self.sim_seen.add(sh)
...
def write_csv(rows: Iterable[Dict[str, Any]], out_csv: str) -> str:
    cols = [
        ...
        "tokens","len","lang","path","chunk_id","n_chunks","repo","near_dup"
    ]
