...
def make_records_for_file(
    *,
    ...
    # Optionally extract KQL blocks from markdown
    ext = Path(rp).suffix.lower()
    if kql_from_markdown and ext in {".md", ".mdx"}:
        blocks = extract_kql_blocks_from_markdown(text)
        records: list[Dict[str, object]] = []
        for idx, b in enumerate(blocks, start=1):
            # Chunk each block (in case a single block is large)
            chunks = chunk_text(b.text, policy)
    ...
    # Default path: chunk the whole file
