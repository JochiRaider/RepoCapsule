...
from .pipeline import run_pipeline, process_items_parallel
...
def convert(config: RepocapsuleConfig) -> Dict[str, int]:
    ...
    jsonl_path = config.sinks.primary_jsonl_name or config.metadata.primary_jsonl
    ...
    if config.qc.enabled:
        ...
        if mode == "inline":
            ...
            qc_summary = _run_post_qc(jsonl_path, config)
            stats["qc"] = qc_summary
