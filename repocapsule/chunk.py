...
"""
Structure-aware text chunking for documentation and code.

This module splits Markdown and reStructuredText into logical blocks, splits
code by line runs, then packs blocks into target-sized token windows with
optional overlap. When available, `tiktoken` is used for token counts; a fast
character-based estimator is used as a graceful fallback.

Public API
----------
- ChunkPolicy
- register_doc_splitter(fmt_name: str, splitter: Callable[[str], list[Block]]) -> None
- split_doc_blocks(text: str, fmt: str | None) -> list[Block]
- chunk_text(text: str, *, mode: str = "doc", fmt: str | None = None,
             policy: ChunkPolicy | None = None, tokenizer_name: str | None = None) -> list[dict]
- count_tokens(text: str, tokenizer: object | None = None, mode: str = "auto") -> int
- detect_fmt_from_lang(lang: str | None) -> str

Behavior
--------
- Docs: recognizes CommonMark-style headings (ATX/Setext) and fenced code blocks,
  and pragmatic reStructuredText titles, directives, and literal blocks, then
  packs them near `target_tokens` with `min_tokens` and `overlap_tokens`.
- Code: uses a simple line-run splitter with blank-line preferences for boundaries.
- Tokenization: prefers `tiktoken` (if present); otherwise uses a heuristic
  chars-per-token estimate tuned for code vs. prose.

Returns
-------
`chunk_text()` yields a list of dicts with keys:
`"text"`, `"n_tokens"`, `"start"`, `"end"`.

Example
-------
>>> from repocapsule.chunk import chunk_text, ChunkPolicy
>>> chunks = chunk_text("# Title\\n\\nBody\\n", policy=ChunkPolicy(target_tokens=50))
>>> chunks[0]["text"].startswith("# Title")
True

Notes
-----
Heuristics for headings and directives follow widely used CommonMark and
Docutils/Sphinx conventions; they are not full parsers but sufficient for
chunk boundaries.
"""
    ...
    for bt, bstart, bend in blocks:
        ...
        if cur_len_tok + b_tokens > target_tokens and cur_len_tok >= min_tokens:
            ...
            if overlap_tokens > 0:
                ...
                tail = tail = _take_tail_chars_for_overlap(cur_text, overlap_tokens, cur_mode)
