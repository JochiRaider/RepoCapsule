...
"""
Quality scoring utilities for RepoCapsule JSONL chunks.

This module computes per-chunk diagnostics and a composite 0..100 score for
RepoCapsule outputs. It blends fast, dependency-light heuristics with optional
language-model perplexity and near-duplicate detection via SimHash and MinHash
LSH. A one-call helper writes results to CSV for triage and reporting.

Signals
-------
- Length band: target token windows tuned by coarse language class.
- Text shape: ascii_ratio, repetition rate (k-grams), and a simple code
  complexity proxy.
- Parse checks: Python (ast), JSON/JSONL, YAML (if available), plus light
  sanity checks for Markdown/reStructuredText, KQL, SPL, Sigma, and YARA.
- Near-duplicates:
  * SimHash (64-bit) checked against a recent window using a Hamming threshold.
  * MinHash + banded LSH to approximate Jaccard similarity at scale.
- Optional LM perplexity: sliding-window PPL with Hugging Face Transformers
  when installed.
- Gopher-style prose heuristics: word-count sufficiency, mean word length,
  symbol and bullet ratios, and ellipsis ratio.

Scoring
-------
Each signal contributes a 0..1 component; a weighted blend yields the final
0..100 score. Near-duplicates receive a soft penalty that scales with
similarity and content type.

Optional dependencies
---------------------
- tiktoken: token counts (falls back to a fast character-based estimate).
- transformers + torch: perplexity for fixed-length LMs.
- pyyaml: YAML parse checks.
The API degrades gracefully if these are missing.

Reproducibility
---------------
MinHash hash families are created from a fixed seed; LSH parameters (bands,
rows, permutations) are configurable. SimHash comparisons use a bounded
recent-window buffer to avoid quadratic comparisons.
...
- class JSONLQualityScorer(
    ...
    • score_record(rec: dict) -> dict
    • score_jsonl_path(jsonl_path: str) -> list[dict]
...
- def write_csv(rows: Iterable[dict], out_csv: str) -> str  # helper

Returned fields (subset)
------------------------
...
near_dup_simhash, near_dup_minhash, minhash_jaccard, minhash_dup_of,
gopher_quality, gopher_flags.

Example
-------
>>> from repocapsule.qc import JSONLQualityScorer, score_jsonl_to_csv
>>> csv_path = score_jsonl_to_csv("repo.jsonl")  # writes repo_quality.csv
>>> scorer = JSONLQualityScorer(lm_model_id="Qwen/Qwen2.5-1.5B",
...                             device="cuda", dtype="bfloat16")
>>> rows = scorer.score_jsonl_path("repo.jsonl")
>>> rows[0]["score"]  # doctest: +ELLIPSIS
...

Notes
-----
- Perplexity uses a sliding window to respect fixed context length.
- SimHash uses a 64-bit signature; the recent-window size controls cost.
- MinHash operates on UTF-8 byte k-grams; LSH bands and rows are tunable.
"""
