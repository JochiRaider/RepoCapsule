...
"""
GitHub helpers for RepoCapsule: URL parsing, repo metadata, and streamed zipballs.

This module provides small, dependency-light utilities to work with GitHub:
it parses common repository URLs into a structured `RepoSpec`, queries the
GitHub REST API for repo/license metadata, downloads repository zipballs
*streamed to disk* (RAM-safe), and iterates zip members with multiple
zip-bomb defenses. It also includes a helper for building safe output
filenames that encode owner/repo, ref, and (optionally) SPDX license.

Public API
----------
- RepoSpec
- parse_github_url(url) -> RepoSpec | None
- github_api_get(path, *, accept=None, timeout=30) -> (status, headers, body)
- get_repo_info(spec) -> dict
- get_repo_license_spdx(spec) -> str | None
- download_zipball_to_temp(spec, *, ref=None, timeout=60,
                           chunk_size=1048576, max_zip_bytes=3221225472) -> str
- iter_zip_members(zip_path, *, max_bytes_per_file=None,
                   max_total_uncompressed=2147483648, max_members=200_000,
                   max_compression_ratio=100.0) -> Iterator[(rel_path, bytes)]
- build_output_basename(spec, *, license_spdx=None, ref=None) -> str

Parameters
----------
max_bytes_per_file : int | None
    Hard cap on bytes read per entry (prevents over-read on large files).
max_total_uncompressed : int
    Upper bound on the sum of actual bytes yielded across entries.
...
def build_output_basename(spec: RepoSpec, *, license_spdx: Optional[str] = None, ref: Optional[str] = None) -> str:
    ...
    if license_spdx:
        parts.append(re.sub(r"[^A-Za-z0-9+.-]", "_", license_spdx.upper()))
