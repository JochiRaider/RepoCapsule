...
"""
RepoCapsule: convert software repositories into JSONL corpora for LLM data prep.

This package exposes a stable, convenience import surface over the core
modules—filesystem traversal, robust decoding, structure-aware chunking,
record assembly, GitHub I/O, PDF extraction (when available), the streaming
pipeline, and high-level runners. Prefer importing from `repocapsule`
directly for common tasks; see `__all__` for the public API.
...
...     "out",
...     policy=ChunkPolicy(mode="doc", target_tokens=1700),
... )

Notes
-----
Optional extras (e.g., PDF and QC helpers) are exported only if their
...
# Converters
from .convert import make_records_for_file, make_records_from_bytes, register_bytes_handler
...
# Runners
from .runner import (
    convert_repo_to_jsonl, convert_repo_to_jsonl_autoname,
...
__all__ = [
    ...
    # convert
    "make_records_for_file", "make_records_from_bytes", "register_bytes_handler"
    # web sources
    "WebPdfListSource", "WebPagePdfSource",  
    # runner
    "convert_repo_to_jsonl", "convert_repo_to_jsonl_autoname", "convert_github_url_to_jsonl", "convert_github_url_to_both", "convert_github_url_to_jsonl_autoname",
    # pipeline
